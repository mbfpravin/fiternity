...
<div id="em-booking" class="em-booking <?php if( get_option('dbem_css_rsvp') ) echo 'css-booking'; ?>">
	...
	?>
	<?php if( !$is_open ): //Double bookings not allowed ?>
	...
	<?php else: ?>
		...
		<h2>Booking form</h2>
		<p>Book your event by filling the form below:</p>
			<div class="form-wrap">
				<form class="em-booking-form form-horizontal" name='booking-form' id="book-form" method='post' action='<?php echo get_bloginfo('template_url') ?>/charge.php/'>
				 	...
				 	<?php
		                ...
		                $amount = filter_var($EM_Ticket->get_price(true), FILTER_SANITIZE_NUMBER_INT);
		            ?>
				 	<input type='hidden' name='amount' id="payamount" value='<?php echo $amount; ?>'/>
				 	<input type='hidden' name='ticketid' id="ticketid" value='<?php echo $EM_Ticket->ticket_id; ?>'/>
				 	<input type='hidden' name='eventname' id="eventname" value='<?php echo $EM_Event->event_name; ?>'/>
				 	<input type='hidden' name='ticketname' id="ticketname" value='<?php echo $EM_Ticket->ticket_name; ?>'/>
					<?php 
						// Tickets Form
						...
						<div class='em-booking-form-details'>
							...
							<div class="em-booking-buttons">
								...
								<?php else: //Display normal submit button ?>
								    <a href="javascript:void(0);"><input type="submit" class="em-booking-submit button" id="em-booking-submit" value="<?php echo esc_attr(get_option('dbem_bookings_submit_button')); ?>" /></a>
								<?php endif; ?>
							</div>
